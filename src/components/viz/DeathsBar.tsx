...
const DeathBars = ({
  ...
  const legendLabels = [
    { label: "Car", pos: 0 },
    { label: "Pedestrian", pos: 54 },
    { label: "Bicycle", pos: 160 },
  ];
  ...
  useEffect(() => {
    ...
    svg
      ...
      .attr("class", (d) =>
        showBreakdown
          ? (colorScale(d.victim_type) as string)
    ...
    svg
      ...
      .call(
        d3
          .axisBottom(xScale)
          .tickSizeOuter(0)
          .tickFormat((_, i) => Array.from(xLabels)[i])
      )
    ...
    d3.select("#deaths-bar-plot svg")
      ...
      .text(
        showBreakdown
          ? "Traffic Deaths by Year and Victim Type"
    ...
    const mousemove = (e: MouseEvent, d: YearlyBreakdown) => {
      const renderHtml = (d: YearlyBreakdown) => {
        const total_killed = d3.sum(
          yearlyBreakdown
            .filter((x) => x.year == d.year)
            .map((x) => x.number_killed)
        );
        const year_str = "<b>" + d.year + "</b><br/>",
          data_str = showBreakdown
            ? d.victim_type + ": " + d.number_killed
            : "Total: <b>" + total_killed + "<b/>";
  ...
  return (
    <>
      <svg width={divWidth} height={divHeight}></svg>
    </>
  );
};

const BarsWithToggle = ({
  yearlyBreakdown,
}: {
  yearlyBreakdown: YearlyBreakdown[];
}) => {
  ...
  return (
    <>
      <div
        className="h-9/10 justify-center items-center flex"
        id="deaths-bar-plot"
      >
        <DeathBars
          yearlyBreakdown={yearlyBreakdown}
      ...
      <div
        className="flex justify-end items-center rounded-md h-1/10 mx-4"
        id="breakdown-toggle"
      >
        <Toggle
